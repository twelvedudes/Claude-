...
def emit_zone_body(zone_mobs: dict, indent: str) -> list:
    """Lua lines for one zone's name -> entries table (no braces)."""
    lines = []

    for name in sorted(zone_mobs):
        lines.append('%s[%s] = {' % (indent, lua_string(name)))

        for e in zone_mobs[name]:
            level_rows = ', '.join(
...
def emit_lua(mobs: dict, source: str) -> str:
    """Monolithic zone -> name -> entries table (single-file mode)."""
    lines = [
        '-- Generated by Whetstone tools/extract_mobs.py - DO NOT EDIT',
        '-- Source: ' + source,
        '-- zone ID -> mob name -> { level range + exact per-level stats }',
        'return {',
    ]

    for zone in sorted(mobs):
        lines.append('    [%d] = {' % zone)
        lines.extend(emit_zone_body(mobs[zone], '        '))
        lines.append('    },')
...
def emit_split(mobs: dict, accounting: dict, out_dir: Path,
               source: str) -> dict:
    """Per-zone files for the 32-bit client: data/mobs/zone_<id>.lua
    plus an index. The addon loads one zone at a time on zone-in
    instead of parsing the whole world into a 32-bit address space.

    Conservation must balance ACROSS the split: the sum of per-zone
    entry counts is asserted against the extraction accounting before
    anything is considered written.
    """
    header = [
        '-- Generated by Whetstone tools/extract_mobs.py - DO NOT EDIT',
        '-- Source: ' + source,
    ...
    for zone in sorted(mobs):
        zone_counts[zone] = sum(len(entries)
                                for entries in mobs[zone].values())

    split_total = sum(zone_counts.values())

    if split_total != accounting['emitted']:
        raise ConservationError(
            'split lost rows: %d emitted by extraction but %d across '
            'zone files' % (accounting['emitted'], split_total))

    out_dir.mkdir(parents=True, exist_ok=True)

    for zone in sorted(mobs):
        lines = list(header)
        lines.append('-- zone %d: mob name -> entries' % zone)
        lines.append('return {')
        lines.extend(emit_zone_body(mobs[zone], '    '))
        lines.append('}')

        (out_dir / ('zone_%d.lua' % zone)).write_text(
            '\n'.join(lines) + '\n', encoding='utf-8')
    ...
    for zone in sorted(zone_counts):
        index_lines.append("        [%d] = { file = 'zone_%d', "
                           'entries = %d },' % (zone, zone,
                                                zone_counts[zone]))
    ...
    (out_dir / 'index.lua').write_text('\n'.join(index_lines) + '\n',
                                       encoding='utf-8')

    return zone_counts


def emit_singletons(mobs: dict, zone_names: dict) -> list:
    """Text lines for exact-stat validation targets: mobs whose spawn
    range is a single level (minLevel == maxLevel) in the starting
    zones - their predicted stats have no level ambiguity at all."""
    starter = [
        'WEST_RONFAURE', 'EAST_RONFAURE', 'NORTH_GUSTABERG',
        'SOUTH_GUSTABERG', 'WEST_SARUTABARUTA', 'EAST_SARUTABARUTA',
        'VALKURM_DUNES',
    ]
    ...
    for name in starter:
        zone = zone_names.get(name)

        if zone is None or zone not in mobs:
            continue

        for mob_name in sorted(mobs[zone]):
            for e in mobs[zone][mob_name]:
                if e['min_level'] == e['max_level']:
                    stats = e['levels'][e['min_level']]
                    lines.append(
                        'zone %d (%s)  %-24s Lv.%d  vit=%d agi=%d '
                        'def=%d eva=%d  (%s %s/%s)%s'
                        % (zone, name.title().replace('_', ' '),
                           mob_name, e['min_level'], stats['vit'],
                           stats['agi'], stats['def'], stats['eva'],
                           e['family'], e['mjob'], e['sjob'],
                           ' NM' if e['nm'] else ''))
...
def main(argv=None) -> int:
    ...
    parser.add_argument('--split', action='store_true',
                        help='treat --out as a directory and write one '
                             'file per zone plus index.lua (32-bit '
                             'client memory)')
    parser.add_argument('--singletons-out', default=None,
                        help='also write exact-stat validation targets '
                             '(minLevel == maxLevel mobs in starting '
                             'zones) to this file')
    ...
    if args.split:
        zone_counts = emit_split(mobs, accounting, out_path, label)
        print('wrote %s/: %d zone files + index, %d mob entries'
              % (out_path, len(zone_counts), total))
    else:
    ...
    if args.singletons_out:
        zone_names = parse_zone_ids(
            Path(args.server) / 'src' / 'map' / 'zone.h')
        lines = emit_singletons(mobs, zone_names)
        Path(args.singletons_out).write_text(
            '\n'.join(lines) + '\n', encoding='utf-8')
        print('wrote %s: %d singleton validation targets'
              % (args.singletons_out, len(lines)))
